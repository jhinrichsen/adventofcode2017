...
| Linux |     Amd64    | AMD Ryzen 7 7840HS w/ Radeon 780M Graphics | 1.3s
|===
...
Optimization: Replaced 2D `[][]byte` grid with 1D slice representation (`Grid1D`) for better cache locality. Combined with uint16 bitset pattern matching to eliminate string conversions and grid copying. Part2 improved 3.6x (107ms → 30ms) with 90% less memory and 99.8% fewer allocations (3.87M → 6.4k).
